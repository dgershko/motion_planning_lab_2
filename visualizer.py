...
class Visualize_UR(object):
    ...
    def draw_manual_spheres(self, spheres: np.ndarray):
        """
        spheres is a list of arrays: [x,y,z,rad]
        """
        u, v = np.mgrid[0:2 * np.pi:10j, 0:np.pi:10j]
        for sphere in spheres:
            x = np.cos(u) * np.sin(v) * sphere[3] + sphere[0]
            y = np.sin(u) * np.sin(v) * sphere[3] + sphere[1]
            z = np.cos(v) * sphere[3] + sphere[2]
